"""
Terradev MCP Tools for CUCo Integration - FIXED VERSION
...
class CUCoMCPTools:
    ...
    def auto_optimize_deployment(self, deployment_id: str, workload_spec: Dict[str, Any], optimization_preferences: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Analyze deployment (synchronous version)
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                optimization_plan = loop.run_until_complete(
                    self.auto_optimizer.analyze_deployment(deployment_id, workload_spec)
                )
                
                # Apply optimizations
                optimization_results = loop.run_until_complete(
                    self.auto_optimizer.apply_optimizations(deployment_id, optimization_plan)
                )
            finally:
                loop.close()
    ...
    def validate_p95_boundaries(self, workload_type: str, metrics: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Validate each metric against P95 boundaries
            validation_results = {
